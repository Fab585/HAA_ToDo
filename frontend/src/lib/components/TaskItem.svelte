<script lang="ts">
  ...
  import { PRIORITY_COLORS, TaskPriority } from "$lib/types/task";
  ...
  // Reactive computed priority color
...
>
  ...
  {#if isSwipingRight}
    <div
      class="absolute inset-0 bg-green-500 flex items-center justify-start px-4"
  ...
  {#if isSwipingLeft}
    <div
      class="absolute inset-0 bg-red-500 flex items-center justify-end px-4"
      transition:fade={{ duration: 150 }}
    >
      <svg class="w-6 h-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path
    ...
    <div class="flex items-start gap-3">
      ...
      <div class="flex-1 min-w-0">
        ...
        <div class="flex items-start gap-2">
          ...
          {#if task.priority > 0}
            <span class="flex-shrink-0 px-2 py-0.5 text-xs font-semibold rounded {priorityColor}">
              !{task.priority}
            </span>
          {/if}
        </div>
